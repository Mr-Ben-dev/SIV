/**
 * Quick test script to trigger the MVP swap
 */
import {
  Args,
  Client,
...
const CONTRACT_ADDRESS =
  'AS12JWBDaz6LAQPzD2qbfW34JBtGx2o97v1MHXZCFYMXjneT5LqxW'; // v20
...
async function main() {
  ...
  // Step 2: Arm guard
  console.log('2️⃣ Arming guard (required for rebalancing)...');
  try {
  ...
  console.log('3️⃣ Triggering FIRST AUTONOMOUS REBALANCE...');
  ...
  try {
    const rebalanceOp = await client.smartContracts().callSmartContract({
      ...
      coins: BigInt(50_000_000), // 0.05 MAS storage fee
    });

    console.log('📋 Rebalance operation:', rebalanceOp);

    console.log('\n⏳ Waiting for operation to be final...');
    await client
      .smartContracts()
    ...
    console.log('   1. BalancesRead - Shows current token balances');
    console.log('   2. DriftCalculated - Current allocations and drift');
    console.log('   3. SwapExecuted - If drift > 500 bps');
    console.log('   4. RebalanceExecuted - Confirms completion');
    console.log('   5. AutonomousRebalanceScheduled - NEXT rebalance ⭐');
    console.log('\n🔗 Check Activity page in frontend to see events!');
    console.log(
      '🔗 Or check Massa Explorer:',
      `https://explorer.massa.net/mainnet/operation/${rebalanceOp}`,
    );
    console.log('\n🎉 TEST COMPLETE!');
    console.log('\n📝 Next Steps:');
